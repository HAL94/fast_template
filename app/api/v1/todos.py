...
@todos_router.get("/")
async def get_toods(session: AsyncSession = Depends(get_async_session)) -> AppResponse[PaginatedResult[TodoBase]]:
...
async def get_a_todo(todo_id: UUID, session: AsyncSession = Depends(get_async_session)) -> AppResponse[TodoWithTasks]:
    service = TodoService(session=session)
...
async def create_todo(
    ...
    service = TodoService(session=session)
    result = await service.create_todo(todo)
    return AppResponse(data=result)


@todos_router.delete("/{todo_id}")
async def delete_todo(
    todo_id: UUID, session: AsyncSession = Depends(get_async_session)
) -> AppResponse[Optional[TodoWithTasks]]:
    service = TodoService(session=session)
    result = await service.delete_todo(todo_id)
    return AppResponse(data=result)
